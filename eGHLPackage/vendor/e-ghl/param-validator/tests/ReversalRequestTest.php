...
use eGHL\ParamValidator\ReversalRequest;

class ReversalRequestTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testExtendsBase(){
        $parent = get_parent_class('eGHL\ParamValidator\ReversalRequest');
    ...
    public function testAllCorrectParams(){
        ...
        $this->initParams();
        try{
            $Request = new ReversalRequest($this->params);
            $Request->validate();
        }
        catch(\Exception $e){
    ...
    public function testTransactionTypeError(){
        ...
        // MaxLength Exception
        $this->params[$p_nane] = $this->generateRandomString(5,'A')."ABC";
        $exceptionCount += $this->RunValidator();

        // DataType Exception
        $this->params[$p_nane] = $this->generateRandomString(2,'A')."123";
        $exceptionCount += $this->RunValidator();
